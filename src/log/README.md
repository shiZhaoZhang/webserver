# 基于Ring Log做了一些修改
+ 不能简单使用智能指针，因为单例模式要求构造函数和析构函数是私有，这就导致智能指针无法调用其构造函数和析构函数。
+ 对写线程使用isOpen判断而不是死循环，实现在LOG_CLOSE中能等待所有东西写完之后退出。
+ 不对写线程使用分离，而是在close()函数中使用pthread_join负责收集，防止主线程先退出而写线程还在运行的情况。

# Ring Log [git@github.com:LeechanX/Ring-Log.git]
