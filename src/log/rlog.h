...
//不是线程安全的
/*
如果线程A、B同时调用utc_timer::get_curr_time;
A执行到reset_utc_fmt();停住1s
B执行结束
B获取的时间是B的实际时间，A获取到的时间是B修改的时间
所以线程获取到的时间是直到获取utc_fmt为止
*/
struct utc_timer
{
    utc_timer()
...
public:
    ...
    void set_wid(pthread_t id){ _write_pthread_id = id; }

    void close_log() {
        _isOpen = false; 
        pthread_join(_write_pthread_id, nullptr);
        fclose(_fp);
    }
private:
    ring_log();
...
#define LOG_INIT(log_dir, prog_name, level) \
    ...
    { \
        ...
        ring_log::ins()->set_wid(tid);\
    } while (0)

#define LOG_CLOSE() \
    do \
    { \
        ring_log::ins()->close_log(); \
    } while(0)
